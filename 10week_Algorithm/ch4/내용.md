...
#### 1. 만들 수 있는 경우의 수 
1 ~ n개의 점들을 분리할 수 있는 모든 경우의 수를 `비트마스킹`으로
``` cpp
...
3. 2가지 영역으로 나누어지는 것을 확인
``` cpp
// 1. 만들어진 그룹의 수가 2개여야 한다.
if(compa.first + outa.first == n){

}

/**
 * 2그룹의 인원수를 합한 것이 전체 인원 수이면
 * 조건에 맞게 2그룹으로 나뉜것 
*/
...
## 1. 그래프 (이원 배열) 대칭
<img src="img/14980.jpg" width="700" height="500"/>

그래프 1개 더 생성해서 대칭으로 넣기 
``` cpp
for(int i = 0; i<n; i++){
    for(int j = 0; j< n; j++){
        scanf("%d", &graph1[i][j]);
...
## 2. cnt 변수 1개 이용하는 로직 
<img src="img/14980logic.jpg" width="800" height="300"/>
...
## 마스킹 
#### 1. word 
n개의 문자열 집합을 `word[i]`로 표현했다.
word가 0 이면 

#### 2. (word & mask) == word 
전체 비트들을 마스킹한 결과가 `word`여야 읽을 수 있는 것
...
기존에는 
1. dfs로 개수 파악 
2. 2중 for문으로 visited[r][c] = 개수를 넣음 -> `O(n^2) `
``` cpp

if(!visited[r][c]){ // 방문해야 함 
    cnt ++; // 영역 번호 
    areaSize[cnt] = dfs(r,c, cnt); // 영역 표시 + 개수 저장을 한번의 단계로 진행한다. 
}
```
