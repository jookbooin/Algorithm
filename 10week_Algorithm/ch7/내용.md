...
## graph 모양
* 가로 세로는 (r, c) 해당점만 0
* 대각선은 (r-1, 0), (r, c-1) (r, c) 모두 0이어야 함
``` cpp
...
```
## 점화식 
dp[r][c][dir] : `dir 모양`으로 (r, c)에 도달한 경우
...
## dfs 
* for 문에서 유효성 검사 → `continue`로 접근 못하게
    ``` cpp
        for(int i = 0; i<= 4; i++ ){
...
## 무한히 지나는 점 
이미 지난점을 또 지나게 되면 무한 순회가 될 확률이 높다.
visited를 체크해주어야 하므로 원복하는 과정도 신경 써 주어야 한다.
``` cpp
int dfs(int cr, int cc){
    
    // 이미 지난적 있을 때 또 방문하게 되면 안됨
    if(visited[cr][cc]){
        cout << -1 << endl;
        exit(0);
    }
