...
priority_queue에 정렬 기준을 넣으면 `반대로` 나온다.
``` cpp
struct cmp{
...
```
일반적으로 cmp는 `오름차순`을 나타낼때 사용하는 비교 메서드인데
이를 우선순위큐에 넣게되면

int형을 기준으로 내림차순으로 나오게 된다.
``` cpp
...
struct Ob{
    ...
    bool operator<(const Ob& ob) const{
        if(this->d != ob.d){
            return this->d > ob.d;  // 내림차순 -> pq 오름차순
        }else{
            return this->p < ob.p;  // 오름차순 -> pq 내림차순 
        }
    }
};
...
```
#### [문제해설]
1. 허용되는 d 범위 안에서 가장 큰 p들만 구하면 된다.
2. pq.size는 v[i].first의 개수를 넘을 수 없다. 
...
# 5-B 9935
폭발, 짝짓기면 스택
st.top() 과 정답 문자열의 끝이 같으면 정답 문자열 길이만큼 스택에서 뺀다.
...
#### 이터레이터
* begin() : 문자열의 첫번째 요소를가리키는이터레이터를반환
* end() : 문자열의 마지막 요소 그다음을 가리키는 이터레이터를 반환

#### 문자열 분리
* substr( 시작 위치, `크기` )

#### erase
* string → erase( 시작 위치, `크기` )
* vector 
    ``` cpp
        // 하나의 요소를 지울때
...
# 5-F 1202
n, k 모두 30만이 넘어가므로 O(nlogn) 방식 찾아야 함

## 우선순위큐 
우선순위큐는 기본으로 최대 힙으로 되어있음.
`최대`, `최소` 값만을 구할 때 O(logn) 으로 찾기 쉽다.
